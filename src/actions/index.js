...
export const actFetchProductRatingRequest = () => {
    return async dispatch => {
        const res = await callApi('api/rating', 'GET', null)
        if (res) {
            console.log(res.data)
            dispatch(actFetchProductRating(res.data))
        }
    }
}

export const actFetchProductRating = (dataRating) => {
    return {
        type: Types.FETCH_PRODUCT_RATING,
        dataRating
    }
}

export const actRatingProductRequest = (dbRating) => {
    return async dispatch => {
        const res = await callApi('api/rating', 'POST', dbRating)
        if (res) {
            // console.log(res.data)
            dispatch(actRatingProduct(res.data))
        }
    }
}

export const actRatingProduct = (dbRating) => {
    return {
